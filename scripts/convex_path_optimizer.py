...
class ConvexPathOptimizer:
    ...
    def optimize(self, original_path: list, n_intermediate: int = 3,
        ...
        # 若初始猜測落在障礙物內，側推到安全位置使線性化方向有效
        # 沿路徑方向的法向量（XY 平面內旋轉 90°）
        path_xy = (wps_in[-1] - wps_in[0])[:2]
        path_len = np.linalg.norm(path_xy)
        if path_len > 1e-6:
            path_xy /= path_len
        lat = np.array([-path_xy[1], path_xy[0], 0.0])   # 左側法向

        for i, pt in enumerate(init_pts):
            pt = np.array(pt, dtype=float)
            for obs in self.obstacles:
                if not isinstance(obs, SphereObstacle):
                    continue
                safe_r = obs.radius + self.drone_radius + obs.margin
                d_norm = np.linalg.norm(pt - obs.center)
                if d_norm < safe_r + 0.15:
                    # 推到左側安全距離外，線性化方向就會指向側方
                    init_pts[i] = obs.center + lat * (safe_r + 0.5)
                    init_pts[i][2] = pt[2]
                    break
